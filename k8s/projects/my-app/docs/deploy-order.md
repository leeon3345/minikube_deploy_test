...
- 신규: `k8s/projects/my-app/services/<새서비스폴더>` 커밋만 하면 Argo CD가 Application을 자동 생성
...
```bash
kubectl apply -f k8s/projects/my-app/argo-project.yaml -n argocd
```

- `my-app-project` 프로젝트를 먼저 생성합니다.
- 이 단계를 건너뛰면 ApplicationSet sync 시 "project not found" 오류가 발생합니다.

### 2단계 — ApplicationSet 생성

```bash
kubectl apply -f k8s/projects/my-app/argo-appset.yaml -n argocd
```

- Argo CD가 `k8s/projects/my-app/services/*` 경로를 폴더 단위로 감시합니다.
...
```bash
kubectl apply -f k8s/projects/my-app/base/resource-quota.yaml -n default
```
...
mkdir -p k8s/projects/my-app/services/<new-service>
# deployment.yaml, service.yaml 등 배치

# 2. Git Push
git add k8s/projects/my-app/services/<new-service>
...
수동 sync:
```bash
argocd app sync my-app-<서비스명> --server <ARGOCD_SERVER>
```
...
1. **Argo CD 배포 상태 확인**
   ...
   argocd app wait my-app-<서비스명> --health
   # 동기화 문제가 있을 경우 강제 동기화 수행
   argocd app sync my-app-<서비스명> --force
   # 또는 전체 애플리케이션 상태 확인
