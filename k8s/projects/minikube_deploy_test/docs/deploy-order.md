...
- 신규: `k8s/projects/minikube_deploy_test/services/<새서비스폴더>` 커밋만 하면 Argo CD가 Application을 자동 생성
...
```bash
kubectl apply -f k8s/projects/minikube_deploy_test/argo-appset.yaml -n argocd
```

- Argo CD가 `k8s/projects/minikube_deploy_test/services/*` 경로를 폴더 단위로 감시합니다.
...
```bash
kubectl apply -f k8s/projects/minikube_deploy_test/base/resource-quota.yaml -n default
```
...
mkdir -p k8s/projects/minikube_deploy_test/services/<new-service>
# deployment.yaml, service.yaml 등 배치

# 2. Git Push
git add k8s/projects/minikube_deploy_test/services/<new-service>
...
└── projects/
    └── minikube_deploy_test/                       # 팀별 독립 경로 (Multi-tenant)
